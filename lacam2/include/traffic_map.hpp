...
struct TrafficMap {
  ...
  TrafficMap(const Graph* _G, const int _num_of_agents)
      : G(_G),
  ...
  {
    // For a grid: each vertex has up to 4 outgoing edges (right, left, down,
    // up) Assign each direction a slot: 0=right, 1=left, 2=down, 3=up
  ...
  std::tuple<double, double> get_recorded_regret_cost(uint a, uint b) const
  {
    ...
    // return { std::sqrt(( edge_flow[edge_idx] + 1) * edge_flow[edge_idx2]),
    // (vertex_flow[b]) / 2 };
  ...
  void reset_regret_cost()
  {
    ...
    // std::fill(normalized_PIBT_regret_flow.begin(),
    // normalized_PIBT_regret_flow.end(), 0);
  }
  ...
  void reset()
  {
    ...
    std::fill(normalized_incremental_flow.begin(),
              normalized_incremental_flow.end(), 0);
  }
  ...
  void snapshot_flow_map()
  {
    ...
    if (snapshot_PIBT_regret_flow.size() == PIBT_regret_flow.size()) {
      ...
      std::memcpy(snapshot_normalized_PIBT_regret_flow.data(),
                  normalized_PIBT_regret_flow.data(),
  ...
  std::tuple<double, double> get_traffic_cost_contra_flow_version(uint a,
                                                                  uint b) const
  {
    if (a == b) return {0, 0};
    // if(a == b) return {0, 0}; // No cost if the same vertex
    ...
    //  return { edge_flow[edge_idx] ,  (vertex_flow[b])/2};
    // return { edge_flow[edge_idx],  (vertex_flow[b])/2};
    return {(edge_flow[edge_idx] + 1) * (edge_flow[edge_idx2]),
            (vertex_flow[b]) / 2};
  }
  ...
  void add_incremental_flow_path_from_time_index(const std::vector<uint>& path,
                                                 uint start_time_index)
  {
  ...
  void initialize_traffic_map(
      const std::vector<std::vector<uint>>& agents_paths)
  {
  ...
  void print_incremental_flow(const std::string& flow_filename)
  {
    ...
    for (size_t i = 0; i < G->U.size(); ++i) {
      ...
      for (const auto& neighbor : G->U[i]->neighbor) {
        ...
        vout << i << "," << neighbor_index << "," << incremental_flow[edge_idx]
             << "\n";
      }
    }
    vout.close();
  ...
  //       uint neighbor_index = neighbor->index;
  //       vout << i << "," << neighbor_index<<"," <<
  //       get_regret_cost(i,neighbor_index)<<"\n";
  //     }
  //   }
  //   vout.close();
  // }
  ...
  void smooth_by_jacobi(const std::vector<double>& w0, double lambda, int iters,
                        std::vector<double>& w)
  {
      ...
      for (size_t a = 0; a < G->U.size(); ++a) {
        ...
        for (const auto& nb_v : G->U[a]->neighbor) {
          ...
          if (G->U[b] != nullptr) {
            ...
            int wait_b =
                edge_waiting_index(static_cast<uint>(b), static_cast<uint>(b));
        ...
        int e_wait =
            edge_waiting_index(static_cast<uint>(a), static_cast<uint>(a));
  ...
  void export_PIBT_regret_flow_csv(const std::string& filename) const
  {
    ...
    for (size_t i = 0; i < expected; ++i) {
      fout << i << ',' << snapshot_PIBT_regret_flow[i] << ','
           << snapshot_normalized_PIBT_regret_flow[i] << ',' << num_of_agents
           << '\n';
    }
    fout.close();
  }
